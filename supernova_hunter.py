...
def extract_dna_from_df(df_raw, benchmarks, target_date, rank_name="UNKNOWN", market="KR"):
    try:
        ...
        if market == 'KR':
            idx_c = benchmarks['KR'][benchmarks['KR'].index <= target_date].tail(250)['Close'].values
            safe_idx = min(dday_idx, len(idx_c) - 1)
            idx_20 = idx_c[max(0, safe_idx-20)] if safe_idx >= 20 else idx_c[0]
            idx_ret = ((idx_c[safe_idx] - idx_20) / idx_20) * 100 if idx_20 > 0 else 0.0001
            ...
            spy_c = benchmarks['SPY'][benchmarks['SPY'].index <= target_date].tail(250)['Close'].values
            qqq_c = benchmarks['QQQ'][benchmarks['QQQ'].index <= target_date].tail(250)['Close'].values
            safe_idx_spy = min(dday_idx, len(spy_c) - 1)
            safe_idx_qqq = min(dday_idx, len(qqq_c) - 1)
            spy_20 = spy_c[max(0, safe_idx_spy-20)] if safe_idx_spy >= 20 else spy_c[0]
            qqq_20 = qqq_c[max(0, safe_idx_qqq-20)] if safe_idx_qqq >= 20 else qqq_c[0]
            spy_ret = ((spy_c[safe_idx_spy] - spy_20) / spy_20) * 100 if spy_20 > 0 else 0.0001
            qqq_ret = ((qqq_c[safe_idx_qqq] - qqq_20) / qqq_20) * 100 if qqq_20 > 0 else 0.0001
