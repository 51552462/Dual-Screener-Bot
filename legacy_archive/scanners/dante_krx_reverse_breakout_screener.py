...
# 💡 2. 본캐 팩트 리포트 (해시태그 파싱 오류 및 스팸 방지를 위해 안전하게 교체) — Gemini 캐시·다중키: gemini_report_cache
...
def compute_inverse_1d(df_raw: pd.DataFrame):
    ...
    c, o, h, l, v = df['Close'].values.astype(float), df['Open'].values.astype(float), df['High'].values.astype(float), df['Low'].values.astype(float), df['Volume'].values.astype(float)
...
def scan_market_1d():
    ...
    print(f"\n✅ [한국장 3번 역매공파 스캔 완료] 포착: {tracker['hits']}개 | 소요시간: {elapsed/60:.1f}분\n")
    if SEND_TELEGRAM:
        if tracker["hits"] > 0:
...
def run_scheduler():
    ...
    while True:
        ...
        try:
            import ops_logger
            ops_logger.record_heartbeat("scanner.krx_reverse")
        except Exception:
            pass
