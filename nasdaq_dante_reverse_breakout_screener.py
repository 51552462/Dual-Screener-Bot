...
def compute_inverse_1h(df_raw: pd.DataFrame):
    ...
    sig_type = "💥 연속 P" if isSubsequentSignal else "🎯 첫 P"

    return True, sig_type, df, {"close": c[-1]}
...
def compute_aligned_1d(df_raw: pd.DataFrame):
    ...
    sig_type = "🎯 J (448 완성)" if s3[-1] else "🎯 J (224 상태)" if s2[-1] else "🎯 J (112 상태)"
...
def scan_market(timeframe: str):
    ...
    logic_name = "P (1시간봉)" if timeframe == '1h' else "J (일봉)"
...
def run_scheduler():
    ...
    print("🕒 [7번 봇: US 듀얼 하이브리드 대기 모드]")
    
    while True:
        now_ny = datetime.now(ny_tz)
        
        if now_ny.minute == 27 and (9 <= now_ny.hour <= 15) and now_ny.hour != 14:
            print(f"🚀 [US P/1H 스캔 시작] 미국 현지시간: {now_ny.strftime('%Y-%m-%d %H:%M:%S')}")
            scan_market('1h')
            time.sleep(60) 
