...
## Mirror — 2026-08-09 · [CAT-E] · CAT-E-BARS-01 Reality Audit 검증

### 로컬 구조 이해
- 맞게 반영된 점: exit_type(코드) vs exit_reason(한글) 구분 정확, 로컬 DB 0행 진단으로 VPS SQL 의존성 명확화
- 빠졌거나 불일치: 없음 — 조사 범위·방법 모두 이번 세션 요청과 일치

### 추가 제안
| # | 제안 | Layer | 우선순위 | 근거 |
|---|------|-------|----------|------|
| 1 | bars×ret은 스크립트 대신 §2 SQL 버킷 쿼리 1개로 대체 | 🟢 E | 최고 | Cursor 자체 결론("VPS SQL만으로 즉시 가능")과 일치, 신규 코드 불필요 |
| 2 | I-2 슬롯 병목 계측은 F-GATE-01/F-RETIRE-02 배포 큐 뒤로 순연 | 🟡 F | 중간 | 배포 승인 병목 중복 방지 (08-07 Mirror와 동일 패턴) |
| 3 | RL 연장 식별 컬럼은 가설 생기기 전까지 보류 | 🟢 D/E | 낮음 | 스키마 additive는 근거 있을 때만 |

### 다음 Handoff 후보 재정렬
1. F-GATE-01 · F-RETIRE-02 서버 배포 (디렉터, 병렬)
2. VPS SQL (a)~(d) 결과 회신 → bars×ret 해석
3. (연기) F-QUOTA-LOG-01 — 배포 큐 클리어 후

### 디렉터 한 줄
> CAT-E-BARS-01 조사 자체는 끝 — 여기서 코드를 더 늘리지 말고 VPS SQL로 숫자부터 채운다.

---

## Mirror — 2026-08-09 · [CAT-C] · C-FUNNEL-01 조사 검증 + C-FUNNEL-02 Handoff

### 로컬 구조 이해
- 맞게 반영된 점: 탈락 로그가 **집계만 남고 심볼 단위 미기록**이라는 진단 정확 — `insert_scan_funnel_snapshot` 누락으로 집계조차 무음 실패 중이었던 것도 코드로 확인(회귀, 신규 버그 아님)
- 빠졌거나 불일치: 컷오프(`eff_cos_cutoff`/`eff_ml_cutoff`)가 **일별 동적**이라는 사실이 조사엔 언급만 되고 계측 설계엔 없었음 — 스냅샷 미기록 시 "그날 컷오프가 얼마였는지" 영구 유실되므로 본 Handoff에서 필수 컬럼으로 승격

### 추가 제안
| # | 제안 | Layer | 우선순위 | 근거 |
|---|------|-------|----------|------|
| 1 | insert 회귀 수정을 near-miss 계측과 **분리해 먼저 단독 커밋** | 🟢 C | 최고 | 저위험 버그픽스를 계측 설계 승인 대기에 묶어둘 이유 없음 |
| 2 | `drop()` 시그니처 확장은 **DNA_FAIL·LIQUIDITY 호출부부터 점진 적용** | 🟡 C | 중간 | 전 호출부 일괄 개조는 diff·회귀범위 증가 |
| 3 | C-FUNNEL-03(OHLCV/국면 사후 회고 스크립트)은 **14~30영업일 적재 후**로 분리, 지금 설계 안 함 | 🟡 C/B | 낮음 | 데이터 없이 스펙 확정하면 재작업 위험 |

### 다음 Handoff 후보 재정렬
1. **C-FUNNEL-02**(본 세션 확정) — 계측만, C-1과 병렬 착수 가능
2. **C-FUNNEL-03**(14~30영업일 후) — OHLCV T+1/T+5 회고 + 국면 asof, 신규 스크립트 1개

### 디렉터 한 줄
> 탈락 로그가 지금까지 사실상 없었다는 게 이번 조사의 핵심 — 계측부터 깔고, 알파 판정은 데이터가 쌓인 뒤에 한다.
