...
# ⭐️ 트레이딩뷰 직접 연동 모듈 (1시간봉 데이터 무결성 확보)
try:
    from tvDatafeed import TvDatafeed, Interval
except ImportError:
    print("❌ tvdatafeed 라이브러리가 없습니다. 터미널에 'pip install --upgrade tvdatafeed' 를 입력하세요.")
    os._exit(1)
...
# ================== 트레이딩뷰 고속 통신망 (1시간봉 전용) ==================
TV_POOL_SIZE = 5
tv_pool = queue.Queue()

def initialize_tv_pool():
    print(f"\n🔌 1시간봉 전용 트레이딩뷰 통신망({TV_POOL_SIZE}채널)을 구축 중입니다...")
    for _ in range(TV_POOL_SIZE):
        try: tv_pool.put(TvDatafeed())
        except: pass
    print("✅ 트레이딩뷰 통신망 구축 완료!\n")
...
def compute_bobgeureut(df_raw: pd.DataFrame):
    ...
    df['Senkou1'] = senkou1
    df['Senkou2'] = senkou2
    df['CloudTop'] = cloudTop
    df['BB_Upper'] = bbUpper

    # 4. 횡보(박스권) 조건 (표본표준편차 ddof=1)
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        condBox6m = (std120_shifted / mean120_shifted) < 0.20
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        condBox3m = (std60_shifted / mean60_shifted) < 0.20
    ...
    # ⭐️ 정배열 여부에 따른 시그널 분리 (파인스크립트 100% 동기화)
    ...
    if cat2_Strong[-1]: sig_type = "💥Cat2(정배열) - 120봉 횡보"
    elif cat1_Strong[-1]: sig_type = "💥Cat1(정배열) - 60봉 횡보"
    elif cat2_Normal[-1]: sig_type = "Cat2(Buy V5) - 120봉 횡보"
    elif cat1_Normal[-1]: sig_type = "Cat1(Buy V5) - 60봉 횡보"
...
# ================== 🚀 엔진 A: 1시간봉 (트레이딩뷰 직결 엔진) ==================
def scan_krx_1h():
    clear_old_charts()
    stock_list = get_krx_list_kind()
    if stock_list.empty: return
    
    print(f"\n🇰🇷 [한국장 1H] 트레이딩뷰 직결 엔진 가동! ({len(stock_list)}종목)")
    t0 = time.time()
    ...
    _save_state(state)
    print(f"\n✅ [1H 스캔 완료] 탐색: {tracker['scanned']}개 | 정상 분석: {tracker['analyzed']}개 | 포착: {tracker['hits']}개 | 소요시간: {(time.time() - t0)/60:.1f}분\n")

# ================== 🚀 엔진 B: 일봉 (야후 파이낸스 벌크 엔진) ==================
def scan_krx_1d():
    clear_old_charts()
    stock_list = get_krx_list_kind()
    if stock_list.empty: return
    
    print(f"\n🇰🇷 [한국장 1D] 야후 벌크 다운로드 엔진 가동! ({len(stock_list)}종목)")
    t0 = time.time()
    tracker = {'scanned': 0, 'analyzed': 0, 'hits': 0}
    state = _load_state()
    today_str = datetime.now(pytz.timezone('Asia/Seoul')).strftime("%Y-%m-%d")
    ...
    _save_state(state)
    print(f"\n✅ [1D 스캔 완료] 탐색: {tracker['scanned']}개 | 정상 분석: {tracker['analyzed']}개 | 포착: {tracker['hits']}개 | 소요시간: {(time.time() - t0)/60:.1f}분\n")
