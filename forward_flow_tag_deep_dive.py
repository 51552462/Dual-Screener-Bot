...
# ===========================================================================
# 👑 [독성 철통 방어 1] 어설픈 타협(0.5) 폐기 및 원천 차단(0.0)
# 기계가 "이 태그는 독성이다"라고 판정했다면, 배팅을 줄여서 들어가는 
# 타협은 없습니다. 즉각적으로 비중을 0으로 만들어 진입을 파괴합니다.
# ===========================================================================
FLOW_TAG_TOXIC_DEFAULT_MULT_FALLBACK = 0.0 # 👑 [수정] 0.5 -> 0.0 (Hard Block)
    ...
    *,
    ...
    for key, entry in registry.items():
        ...
        if age_days < 0 or age_days > decay_days:
            continue
        # ===========================================================================
        # 👑 [독성 철통 방어 2] 레지스트리에 등록된 독성은 무조건 0.0 처리
        # 기존 레지스트리에 0.5나 0.8로 약하게 저장된 독성 태그들이라도, 
        # 이 로직을 타는 순간 모조리 0.0(강제 차단)으로 오버라이드 해버립니다.
        # ===========================================================================
        try:
        ...
        if mult < 1.0: 
            mult = 0.0 # 👑 [수정] 1.0 미만의 어떤 페널티라도 무조건 0.0으로 강제 하드 블락
