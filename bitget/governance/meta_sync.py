...
def _run_bitget_meta_governor_cycle() -> str:
    """코인 forward DB + regime → MetaGovernor 스냅샷 갱신."""
    from bitget.infra import config_manager

    cfg = config_manager.load_system_config() or {}
    rk = normalize_regime_key(cfg.get("CURRENT_REGIME_KEY"))
    if rk in ("", "UNKNOWN"):
        rk = normalize_regime_key((cfg.get("REGIME_ANALYSIS") or {}).get("regime_key"))

    state = _default_meta_state()
    prev = load_bitget_meta_unified()
    if isinstance(prev, dict):
        state.update(prev)

    now = datetime.now(timezone.utc).isoformat()
    action = _action_for_regime(rk)

    state["META_REGIME_KEY"] = rk
    state["META_REGIME_CONFIDENCE"] = 0.85 if rk not in ("", "UNKNOWN") else 0.0
    state["META_REGIME_ACTION"] = action
    state["META_GOVERNOR_LAST_RUN_AT"] = now
    state["META_GOVERNOR_LAST_RUN_STATUS"] = "OK"
    state["META_GOVERNOR_INPUT_HASH"] = f"bitget:{market_data_db_path()}:{int(time.time())}"

    kelly_cap = action.get("kelly_cap")
    if kelly_cap is not None:
        try:
            cfg["DYNAMIC_KELLY_RISK"] = float(kelly_cap)
            config_manager.set_config_value("DYNAMIC_KELLY_RISK", float(kelly_cap))
        except (TypeError, ValueError):
            pass
