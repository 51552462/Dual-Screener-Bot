...
def oms_place_limit_order(
    ...
    *,
    ...
    """
    Limit order at ``price`` with clientOid idempotency on transient network errors.
    Returns ok, order_id, client_order_id, raw, filled, remaining, status, message.
    """
    if meta_kill_switch_active():
        return {
    ...
    try:
        ...
        msg = str(detail.exc if detail else e)
        kind = detail.kind.value if detail else "exhausted"
        logger.warning("OMS create_limit_order exhausted kind=%s: %s", kind, msg)
        return {
            "ok": False,
            "order_id": "",
            "client_order_id": used_oid,
        ...
        logger.warning("OMS create_limit_order fatal: %s", e)
        return {
            "ok": False,
            "order_id": "",
            "client_order_id": used_oid,
