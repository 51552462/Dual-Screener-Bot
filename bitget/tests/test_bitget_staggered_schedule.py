...
class TestBitgetStaggeredSchedule(unittest.TestCase):
    def test_spot_slot_count_and_24h_spread(self):
        self.assertEqual(len(SPOT_SCAN_SLOTS), 14)
    ...
    def test_futures_slot_count_no_master(self):
        self.assertEqual(len(FUTURES_SCAN_SLOTS), 13)
    ...
    def test_no_stock_minute_collision_and_no_simultaneous_markets(self):
        # Two-Track air-gap(전용 코인 서버) 완료 후에는 주식 크론과의 분(minute)
        # 충돌 회피가 더 이상 필요 없다(bitget_scan_schedule.py 모듈 docstring 참조).
        # 여기서는 여전히 유효한 불변식만 검증한다:
        # SPOT/FUTURES 는 같은 (시,분)에 동시 실행되면 안 됨(서버 부하/락 경합),
        # 그리고 reconcile 크론(매시 :53)과도 겹치면 안 된다.
        for s in ALL_SCAN_SLOTS:
            self.assertNotEqual(s.minute, 53, f"{s.mode} collides with reconcile (:53)")
