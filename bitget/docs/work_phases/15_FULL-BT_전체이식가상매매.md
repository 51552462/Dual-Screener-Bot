...
| **FULL-BT-HIST-2** | engine_hit / gate_reject 진단 계측 (`full_bt_diag`) | 🟢 | **Claude OK** · 엔진 미히트 재판정 |
| **FULL-BT-HIST-3** | call/outcome/tf_coverage 분리 계측 | 🟢 | Claude 부분반려·디렉터(A) |
| **FULL-BT-HIST-3-FIX** | warmup fetch-range 교정 (harness Adapter) | 🟡 | **WAIT_CLAUDE_OK** · 전체런 금지 |
