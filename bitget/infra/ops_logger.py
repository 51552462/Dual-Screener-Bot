"""
Bitget 중앙 운영 이벤트: append-only `bitget_ops_events.sqlite` (WAL).

주식 `ops_logger.py`와 동일 3채널:
  - log.python
  - gauge.snapshot
  - heartbeat.tick
"""
...
class OpsEventsSQLiteHandler(logging.Handler):
    def __init__(self, default_component: str = "bitget") -> None:
        super().__init__(level=logging.DEBUG)
        self.default_component = (default_component or "bitget")[:128]
...
def install_unhandled_exception_hooks() -> None:
    def _thread_excepthook(args) -> None:  # type: ignore[no-untyped-def]
        logging.getLogger("bitget.unhandled").error(
    ...
    def _sys_excepthook(exc_type, exc, tb) -> None:  # type: ignore[no-untyped-def]
        logging.getLogger("bitget.unhandled").critical(
