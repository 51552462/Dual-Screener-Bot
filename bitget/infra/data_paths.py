...
def data_refresh_lock_path() -> str:
    """Legacy lock file — reset/cleanup only. Writers use runtime_lock_path() SSOT."""
    return os.path.join(bitget_data_dir(), ".bitget_data_refresh.lock")


def job_lock_path(mode: str) -> str:
    """Single writer flock for all pipeline modes (SQLite WAL = one writer at a time)."""
    _ = mode  # kept for call-site clarity / future mode-specific timeouts
